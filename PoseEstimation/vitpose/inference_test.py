...
class VitInference:
    ...
    def inference(self, img: np.ndarray) -> dict[typing.Any, typing.Any]:
        ...
        for bbox, id, score in zip(bboxes, ids, scores):
            ...
            if 0 <= id < len(scores):
                scores_bbox[id] = scores[id] 
    
        # try:
        #     # hips와 head 좌표 조정
        #     #frame_keypoints[-1][6] = ((np.array(frame_keypoints[-1][6])+(np.array(frame_keypoints[-1][7])-np.array(frame_keypoints[-1][6]))*0.2)).tolist()
        #     frame_keypoints[-1][8] = ((np.array(frame_keypoints[-1][7])+(np.array(frame_keypoints[-1][8])-np.array(frame_keypoints[-1][7]))*0.6)).tolist()
        #     frame_keypoints[-1][9] = ((np.array(frame_keypoints[-1][8])+(np.array(frame_keypoints[-1][9])-np.array(frame_keypoints[-1][8]))*0.3)).tolist()
